...
RUN mkdir -p /opt/kubernetes/bin && \
    chown -R 1001:0 /opt/kubernetes && \
    chmod -R g=u /opt/kubernetes
...
RUN curl -sL -o /opt/kubernetes/bin/kubectl-1.15 https://storage.googleapis.com/kubernetes-release/release/v1.15.11/bin/linux/amd64/kubectl && \
...
RUN curl -sL -o /tmp/octant.tar.gz https://github.com/vmware-tanzu/octant/releases/download/v0.10.2/octant_0.10.2_Linux-64bit.tar.gz && \
    tar -C /opt/kubernetes/bin --strip-components 1 -xf /tmp/octant.tar.gz octant_0.10.2_Linux-64bit/octant && \
