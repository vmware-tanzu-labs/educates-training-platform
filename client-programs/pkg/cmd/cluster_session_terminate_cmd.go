...
func (o *ClusterSessionTerminateOptions) Run() error {
	...
	req, err = http.NewRequest("GET", fmt.Sprintf("%s/workshops/session/%s/terminate/", portalUrl, o.Name), nil)
	...
	if res.StatusCode != 200 {
		return errors.New("cannot execute session terminate against training portal")
	}

	resBody, err = io.ReadAll(res.Body)

	if err != nil {
		return errors.Wrapf(err, "cannot read response to termination request")
	}
