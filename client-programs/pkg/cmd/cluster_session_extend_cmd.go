...
func (o *ClusterSessionExtendOptions) Run() error {
	...
	req, err = http.NewRequest("GET", fmt.Sprintf("%s/workshops/session/%s/extend/", portalUrl, o.Name), nil)
	...
	if res.StatusCode != 200 {
		return errors.New("cannot execute session extension against training portal")
	}

	resBody, err = io.ReadAll(res.Body)

	if err != nil {
		return errors.Wrapf(err, "cannot read response to extend request")
	}
