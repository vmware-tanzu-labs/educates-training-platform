$(document).ready(function() {
    ...
    $.each([$('.execute-all .content')], function() {
        if (window.location !== window.parent.location) {
            $(this).find('.highlight').prepend('<span class="execute-glyph fas fa-running" aria-hidden="true"><sup><sup>*</sup></sup></span>');
            this.parent().click(function(event) {
                if (event.shiftKey) {
                    $(this).find('.execute-glyph').removeClass('text-danger');
                    $(this).find('.execute-glyph').addClass('text-success');
